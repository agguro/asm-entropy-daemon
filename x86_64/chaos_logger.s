...
.equ O_RDWR,             0x02
.equ O_RDWR_CREAT,       0x42                   # O_RDWR (0x02) | O_CREAT (0x40) = 0x42 (66 decimal)
.equ S_IRUGO_IWUGO,      0x1B6                  # 0666 octal = 0x1B6 (438 decimal) - rw-rw-rw- permissions
.equ S_IRWXU_GO,         0x1FF                  # 0777 octal = 0x1FF (511 decimal) - rwxrwxrwx permissions
...
_start:
    # -------------------------------------------------------------------------
    # 1. Parse command-line arguments (argc and argv)
    # -------------------------------------------------------------------------
    movq    (%rsp),             %rax            # Load argc from stack top
    cmpq    $2,                 %rax            # Ensure at least 1 user argument was passed
    ...
    leaq    8(%rsp),            %r9             # Pointer to argv[0]
...
.validate_args:
    ...
    # -------------------------------------------------------------------------
    # 2. Record start execution time if statistics enabled
    # -------------------------------------------------------------------------
...
.skip_start_time:
    ...
    # -------------------------------------------------------------------------
    # 3. Ensure 'chaos_dumps' output directory exists
    # -------------------------------------------------------------------------
    movq    $SYS_MKDIR,         %rax
    ...
    # -------------------------------------------------------------------------
    # 4. Open and map shared memory region
    # -------------------------------------------------------------------------
    movq    $SYS_OPEN,          %rax
    ...
    testq   %rax,               %rax
    js      error_exit
    movq    %rax,               %r12            # %r12 = Shared memory base address

    movq    $1,                 4080(%r12)      # Client control state flag

    # -------------------------------------------------------------------------
    # 5. Spawn parallel child worker processes
    # -------------------------------------------------------------------------
    xorq    %r13,               %r13
...
.wait_children_loop:
    movq    $-1,                %rdi            # Wait for any child process
    xorq    %rsi,               %rsi
    movq    $SYS_WAIT4,         %rax
    xorq    %rdx,               %rdx
    xorq    %r10,               %r10
...
.parent_done_waiting:
    ...
    # Compute execution elapsed time in seconds
    movq    time_end_sec(%rip), %rax
    subq    time_start_sec(%rip),%rax

    # Convert integer seconds to ASCII via u64toa
    subq    $32,                %rsp
    movq    %rax,               %rdi            # Integer value to convert
    movq    %rsp,               %rsi            # Target output buffer pointer
    movq    $32,                %rdx            # Buffer capacity length
    call    u64toa
...
# -----------------------------------------------------------------------------
# Worker Process Logic (Child Fork)
# -----------------------------------------------------------------------------
.worker_child_process:
.worker_watchdog_init:
    movq    4088(%r12),         %r14            # Load initial heartbeat counter
...
.worker_heartbeat_advanced:
    ...
    # Allocate stack buffer for dynamic filename: chaos_dumps/dump_[PID].bin
    subq    $128,               %rsp

    # Copy prefix string "chaos_dumps/dump_" into stack buffer
    ...
    # Retrieve current worker Process ID (PID)
    movq    $SYS_GETPID,        %rax
    syscall
    
    movq    %rax,               %rdi            # PID value
    leaq    32(%rsp),           %rsi            # Scratch buffer for ASCII conversion
    movq    $32,                %rdx            # Scratch buffer length
    call    u64toa

    # Copy ASCII PID bytes immediately after prefix (offset 17)
    movq    %rsi,               %r8
    movq    %rdx,               %rcx
    leaq    17(%rsp),           %rdi
...
.copy_pid_loop:
    ...
    movb    $0,                 (%rdi)          # Null delimiter

    # Append ".bin" suffix (4 bytes)
    leaq    file_suffix(%rip),  %rsi
    movl    (%rsi),             %eax
    movl    %eax,               (%rdi)

    # Open / create binary dump output file
    movq    $SYS_OPEN,          %rax
    movq    %rsp,               %rdi
    ...
    testq   %rax,               %rax
    js      error_exit
    movq    %rax,               %r9             # %r9 = Output file descriptor

    # Set dump file size: nums_per_process * 8 bytes
    ...
    # Memory map the output dump file
    movq    $SYS_MMAP,          %rax
    xorq    %rdi,               %rdi
    movq    $PROT_READ_WRITE,   %rdx
    movq    $MAP_SHARED,        %r10
    ...
    testq   %rax,               %rax
    js      error_exit
    movq    %rax,               %r11            # %r11 = Output dump memory base pointer

    xorq    %r15,               %r15            # %r15 = Acquisition counter index
...
.worker_acquisition_loop:
    ...
    xorq    %rbx,               %rbx            # %rbx = Search slot index
...
.worker_scan_slots:
    ...
    movq    $-1,                %rax            # Expected value: Free (-1)
    movq    $0,                 %rdx            # Desired value: Pending (0)
    lock cmpxchgq %rdx,         (%rdi)          # Atomic claim of slot
    jz      .worker_wait_slot_result

    incq    %rbx
    andq    $63,                %rbx            # Wrap around 64 slots
    pause
    testq   $63,                %rbx
...
.worker_wait_slot_result:
    pause
    cmpq    $-1,                (%rdi)          # Wait until engine finishes and sets flag to -1
    jne     .worker_wait_slot_result

    movq    8(%rdi),            %rax            # Read generated 64-bit random value
    movq    %rax,               (%r11, %r15, 8) # Store into mapped file buffer
...
.atoi_loop:
    ...
    testq   %rdx,               %rdx
    jz      .atoi_done
    subq    $0x30,              %rdx            # 0x30 = ASCII '0' -> convert to integer digit
    imulq   $10,                %rax
    addq    %rdx,               %rax
    incq    %rdi
    jmp     .atoi_loop
